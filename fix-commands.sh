...
# Fix listItems calls - change from object parameter to devstepsPath string
...
# Fix updateItem calls - add devstepsPath parameter
sed -i 's/await updateItem({/const devstepsPath = path.join(workspaceFolder.uri.fsPath, '"'"'.devsteps'"'"');\n        const result = await updateItem(devstepsPath, {/g' "$FILE"

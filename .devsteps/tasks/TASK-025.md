...
## Context
SPIKE-002 identified critical performance bottleneck: Dashboard calls `listItems()` 5 times on every load, reading `.devsteps/index.json` redundantly.
...
private async loadAllData() {
  const devstepsPath = path.join(workspaceFolder, '.devsteps');

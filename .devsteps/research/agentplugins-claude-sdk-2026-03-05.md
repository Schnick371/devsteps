...
## Correction — 2026-03-24: SKILL.md as Tool-Reference Library

**Corrected finding:** The `skill-md` entry in the Technology Radar was rated **TRIAL** with rationale "does not replace `.prompt.md` workflow". This is accurate for the _entry-point-stub_ use case, but **undervalues the primary SKILL.md use case**: tool-reference libraries with progressive loading.

Real-world evidence (Copilot Chat 0.40.1, GitLens 17.11.1, Python Envs 1.24.0):
- Skills are used as **reusable procedural knowledge** (debugging workflows, commit audit procedures, pre-commit check sequences), not just slash-command stubs
- The `references/` subfolder pattern enables deep documentation without hitting the 5000-token body limit
- Progressive loading (description → body → references) makes skills efficient for large reference material

**Impact on DevSteps:** The duplicated DevSteps MCP tool knowledge across 48 files should be consolidated into a `devsteps-mcp` skill with reference sub-docs. This is **independent of the plugin distribution blocker** (C-01/C-02) — workspace-local skills at `.github/skills/devsteps-mcp/` work today without any plugin mechanism.

**Updated Technology Radar entry:**

| Signal | Category | Verdict | Rationale |
|--------|----------|---------|-----------|
| `skill-md` (entry-point stub) | Distribution | **TRIAL** | Unchanged — still limited without `prompts:` support |
| `skill-md` (tool-reference library) | Knowledge consolidation | **ADOPT** | Eliminates tool-knowledge duplication across agent/prompt/instruction files; works today as workspace-local skill |

---

*Corrected by coord · 2026-03-24 · Based on analysis of 36 installed SKILL.md files across 4 extensions*

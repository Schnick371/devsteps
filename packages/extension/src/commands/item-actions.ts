  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (node?: string | { item?: { id: string }; label?: string }) => {
        ...
        try {
          itemResult = await getItem(
            path.join(workspaceFolder.uri.fsPath, '.devsteps'),
            targetItemId
          );
        } catch (error) {
          vscode.window.showErrorMessage(
        ...
        const newStatus = await vscode.window.showQuickPick(
          STATUSES.map((s) => ({
            ...s,
            label: s.value === currentStatus ? `${s.label} (current)` : s.label,
          })),
        ...
        try {
          await updateItem(path.join(workspaceFolder.uri.fsPath, '.devsteps'), {
            id: targetItemId,
            status: newStatus.value as ItemStatus,
          });
          treeDataProvider.refresh();
          vscode.window.showInformationMessage(
            `✅ Updated ${targetItemId}: ${currentStatus} → ${newStatus.value}`
          );
        } catch (error) {
          vscode.window.showErrorMessage(
    ...
    vscode.commands.registerCommand(
      ...
      async (node?: string | { item?: { id: string }; label?: string }) => {
        ...
        try {
          ...
          const folder =
            TYPE_TO_DIRECTORY[itemResult.metadata.type as keyof typeof TYPE_TO_DIRECTORY];
        ...
        try {
          ...
          const property = await vscode.window.showQuickPick(
            [
              ...
              { label: '🎯 Priority', value: 'priority', description: item.eisenhower },
          ...
          if (property.value === 'status') {
            ...
            const newTitle = await vscode.window.showInputBox({
              ...
              value: item.title,
              validateInput: (v) =>
                !v?.trim() ? 'Title cannot be empty' : v.length > 200 ? 'Max 200 chars' : null,
            });
            ...
            const np = await vscode.window.showQuickPick(
              [
                ...
                { label: '⚪ Neither', value: 'not-urgent-not-important' },
              ],
              { placeHolder: 'Select priority' }
            );
            if (np)
              updatePayload.eisenhower =
                np.value as import('@schnick371/devsteps-shared').EisenhowerQuadrant;

...
describe('Auto-Migration Module', () => {
	...
	const createMetadata = (id: string, type: ItemType, status: ItemStatus, eisenhower: EisenhowerQuadrant): ItemMetadata => ({
		id,
		type,
		category: 'general',
		title: `Test ${type} ${id}`,
	...
	describe('performMigration', () => {
		it('should throw error when .devsteps directory does not exist', async () => {
			...
			await expect(performMigration(nonExistentDir)).rejects.toThrow('.devsteps directory not found');
		});
		...
		it('should throw error when refs-style index already exists', async () => {
			...
			await expect(performMigration(devstepsDir)).rejects.toThrow('Refs-style index already exists');
		});

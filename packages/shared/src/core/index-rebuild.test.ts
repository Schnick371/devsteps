...
describe('Index Rebuild Operations', () => {
	...
	describe('rebuildIndex - Error Handling', () => {
		...
		it('should throw error when devsteps directory does not exist', async () => {
			...
			await expect(rebuildIndex(nonExistentDir)).rejects.toThrow(
	...
	describe('rebuildIndex - Edge Cases', () => {
		...
		it('should handle items with all possible statuses', async () => {
			const statuses: Array<'draft' | 'planned' | 'in-progress' | 'review' | 'done' | 'blocked' | 'cancelled' | 'obsolete'> = [
				'draft',
				'planned',
				'in-progress',
				'review',
		...
		it('should handle all eisenhower quadrants', async () => {
			const quadrants: Array<'urgent-important' | 'not-urgent-important' | 'urgent-not-important' | 'not-urgent-not-important'> = [
				'urgent-important',

...
import type { ItemType } from '@devcrumbs/shared';
import { TYPE_TO_DIRECTORY } from '@devcrumbs/shared';
...
export default async function exportHandler(args: {
  ...
  if (args.format === 'markdown') {
    ...
    filename = args.output_path || 'devcrumbs-export.md';
  } else if (args.format === 'html') {
    output = generateHTML(config, index.stats, fullItems);
    filename = args.output_path || 'devcrumbs-export.html';
  } else {
    output = JSON.stringify({ config, stats: index.stats, items: fullItems }, null, 2);
    filename = args.output_path || 'devcrumbs-export.json';
  }
...
<head>
  <meta charset="utf-8">
  <title>${config.project_name} - DevCrumbs Export</title>
  <style>

...
function getChannel(): 'pre-release' | 'stable' {
  return packageJson.version.includes('-') ? 'pre-release' : 'stable';
}
...
export default async function statusHandler(args: { detailed?: boolean }) {
  try {
    ...
    const stats = {
      ...
      by_eisenhower: {} as Record<string, number>,
    };

    for (const item of allItems) {
      stats.by_type[item.type] = (stats.by_type[item.type] || 0) + 1;
      stats.by_status[item.status] = (stats.by_status[item.status] || 0) + 1;
      const q: string = (item as { eisenhower?: string }).eisenhower ?? 'not-urgent-important';
      stats.by_eisenhower[q] = (stats.by_eisenhower[q] || 0) + 1;
    }
    ...
    const blockedItems = allItems.filter((item: { status: string }) => item.status === 'blocked');
    ...
    const result: {
      ...
      stats: {
        ...
        by_eisenhower: Record<string, number>;
      };
      warnings?: {
        stale_items: { id: string; title: string; days_since_update: number }[];
      ...
      stats: {
        ...
        by_eisenhower: stats.by_eisenhower,
      },
    };

    if (staleItems.length > 0 || blockedItems.length > 0) {
      result.warnings = {
        ...
        ...(blockedItems.length > 0 ? { blocked_count: blockedItems.length } : {}),
      };
    }

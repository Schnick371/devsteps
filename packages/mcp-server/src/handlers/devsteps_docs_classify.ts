...
export default async function devstepsDocsClassifyHandler(args: Record<string, unknown>) {
	...
	if (scores[winner] > 0) signals.push(`Primary: ${winner} (${(scores[winner] * 100).toFixed(0)}%)`);
	if (secondType && mixed) {
		signals.push(`Runner-up: ${secondType} (${(scores[secondType as keyof typeof scores] * 100).toFixed(0)}%)`);
	}

	// Suggest splits for mixed documents
	const suggestedSplits = mixed && secondType
		? [
				{

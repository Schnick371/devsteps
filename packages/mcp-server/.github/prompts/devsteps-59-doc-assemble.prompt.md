...
- Both `done` → proceed
- Either not `done` → stop, report missing tasks, offer `devsteps-20-start-work` with the blocking task

### Step 3 — Coverage Check

Count BOM nodes with `status: draft` and empty `description`.  
If >10% of nodes are empty placeholders → warn user; suggest `devsteps-57-doc-review` first.  
...
Suggest `devsteps-57-doc-review` if coverage gaps were skipped.

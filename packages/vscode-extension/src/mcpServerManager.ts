...
/**
 * Manages the DevCrumbs MCP server lifecycle
...
export class McpServerManager {
  ...
  async start(): Promise<void> {
    try {
      ...
      this.provider = (vscode as any).lm.registerMcpServerDefinitionProvider('devcrumbs-mcp', {
          ...
          return [
            {
              ...
              args: [mcpServerPath],
              type: 'stdio',
              description: 'DevCrumbs task tracking and AI integration',
            },
          ];
        },
      });
      ...
      // Update status bar
      this.statusBarItem.text = '$(check) DevCrumbs MCP';
      this.statusBarItem.tooltip = 'DevCrumbs MCP Server registered';
      this.statusBarItem.show();

      console.log('DevCrumbs MCP Server registered successfully');
    } catch (error) {
  ...
  private showFallbackConfiguration(): void {
    const message =
      'DevCrumbs MCP Server requires manual configuration. ' +
    ...
    this.statusBarItem.text = '$(warning) DevCrumbs MCP';
    this.statusBarItem.tooltip = 'DevCrumbs MCP Server - Manual configuration required';
    this.statusBarItem.show();
  }
    ...
    this.context.subscriptions.push(
      vscode.commands.registerCommand('devcrumbs.mcp.showStatus', () => {
        const message = this.provider
          ? 'DevCrumbs MCP Server is registered and available in Copilot Chat.'
          : 'DevCrumbs MCP Server requires manual configuration.';

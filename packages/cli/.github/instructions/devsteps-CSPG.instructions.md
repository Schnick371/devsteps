...
- **Guide-cycle per-step Q&A** (devsteps-35-guide-cycle) is a distinct mechanism — NOT a CSPG gate. It has its own continuity loop with mandatory `#vscode_askQuestions` at every step. Blocking issues generate handoff prompts; see the guide-cycle prompt for the full protocol.

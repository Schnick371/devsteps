...
export function checkNode(): CheckResult {
  ...
  if (!version)
    return { name: 'Node.js', status: 'warn', message: 'Could not determine Node.js version' };
...
export function checkPackageManager(): CheckResult {
  ...
  if (preferred)
    return {
      name: 'Package Manager',
      status: 'pass',
      message: `${preferred.name} ${getVersion(preferred.name) || '(version unknown)'}`,
    };
  return {
    name: 'Package Manager',
    status: 'warn',
...
export function checkDevStepsProject(): CheckResult {
  ...
  if (!existsSync(devstepsDir))
    return {
      ...
      fix: 'Run: devsteps init <project-name>',
    };
  ...
  if (!existsSync(configPath) || !hasRefsStyleIndex(devstepsDir)) {
    return {
      ...
      fix: 'Reinitialize: rm -rf .devsteps && devsteps init',
    };
  }
...
export function checkTypeScript(): CheckResult {
  ...
  if (!hasTsConfig)
    return { name: 'TypeScript', status: 'pass', message: 'Not used in this project' };
...
export function checkMCPConfig(): CheckResult {
  ...
  if (existsSync(vscodeConfig))
    return { name: 'MCP Configuration', status: 'pass', message: 'VS Code MCP configured' };
  if (existsSync(cursorConfig))
    return { name: 'MCP Configuration', status: 'pass', message: 'Cursor MCP configured' };
  return {
    name: 'MCP Configuration',
    status: 'warn',
    message: 'Not configured',
    fix: 'Run: devsteps setup',
  };
}

# DevCrumbs Output Channel Empty - RESOLVED (2nd Attempt)
...
```typescript
// WRONG (old):
vscode.window.createOutputChannel('DevCrumbs')

// CORRECT (new):
vscode.window.createOutputChannel('DevCrumbs', { log: true })
```
...
try {
  ...
  logger.warn('No .devcrumbs directory found');
...
```typescript
if (!checkDevCrumbsInitialized(treeDataProvider)) {
  logger.warn('Command invoked but DevCrumbs not initialized');
  vscode.window.showWarningMessage('Please initialize DevCrumbs first');
  return; // Command exits gracefully, not "not found"
}
```

## Results
- ✅ Output channel "DevCrumbs" appears in dropdown
...
- ✅ Logs respect `devcrumbs.logging.level` setting

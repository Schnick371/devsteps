...
async function checkMetadataQuality() {
  ...
  for (const item of allItems) {
    ...
    const descPath = join('.devcrumbs', `${item.type}s`, `${item.id}.md`);
    if (!existsSync(descPath)) {
      issues.push({
        ...
        fix: 'Create description with devcrumbs update'
      });
    }
    ...
    if (item.type === 'task' && (!item.affected_paths || item.affected_paths.length === 0)) {
      issues.push({
        ...
        fix: 'Add with: devcrumbs update TASK-XXX --paths file1.ts,file2.ts'
      });
    }

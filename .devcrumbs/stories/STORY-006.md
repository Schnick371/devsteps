...
## User Value
As a developer, I want `devcrumbs doctor` to find invalid relationships and orphaned items in my existing project, so I can fix data integrity issues before they cause problems.
...
## Solution
Extend `devcrumbs doctor` command with relationship integrity checks using existing validation engine.
    ...
    for (const targetId of targetIds) {
      ...
      if (!target.linked_items[inverse].includes(item.id)) {
        report.push({
          ...
          fix: `Run: devcrumbs link ${targetId} ${inverse} ${item.id}`
        });
      }
    }
  }
...
⚠ TASK-041: Asymmetric link
  ...
  Fix: devcrumbs link EPIC-005 implemented-by TASK-041

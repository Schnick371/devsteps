...
## Problem
Index files (`.devcrumbs/index.json`, type-specific indexes) can become stale or corrupted:
- Item deleted but still in index
- Item created but missing from index
- Duplicate entries
- Wrong category counts

This leads to incorrect `devcrumbs list` results and status reports.
...
async function checkIndexConsistency() {
  ...
  // Load indexes
  const mainIndex = await loadIndex('.devcrumbs/index.json');
  ...
  for (const type of ITEM_TYPES) {
    const typeIndex = await loadIndex(`.devcrumbs/${type}s/index.json`);

# AI Assistant Guide für DevCrumbs

## 🎯 Kern-Prinzip: Hybrid Methodology verstehen

DevCrumbs unterstützt **zwei parallele Hierarchien**:
...
```typescript
// 1. Spike erstellen
devcrumbs-add --type spike --title "Architecture Research" --priority high

// 2. Mit Epic verlinken (NICHT mit Story!)
devcrumbs-link --source SPIKE-001 --relation implements --target EPIC-003

// 3. Optional: Task-Dependency erstellen
devcrumbs-link --source SPIKE-001 --relation required-by --target TASK-004

// 4. Optional: Story informieren
devcrumbs-link --source SPIKE-001 --relation relates-to --target STORY-004
```
...
```typescript
// 1. Story erstellen
devcrumbs-add --type story --title "Feature Implementation"

// 2. Mit Epic verlinken
devcrumbs-link --source STORY-001 --relation implements --target EPIC-001

// 3. Tasks verlinken
devcrumbs-link --source TASK-001 --relation implements --target STORY-001
devcrumbs-link --source TASK-002 --relation implements --target STORY-001
```

### Link-Validierung (vor jedem devcrumbs-link!)
...
### devcrumbs-trace verwenden

```bash
# Vollständiger Baum (alle Ebenen, auch done items!)
devcrumbs-trace EPIC-003 --depth 4
...
Bevor du `devcrumbs-link` aufrufst, prüfe:
...
3. ✅ Gibt es schon einen Link?
   - `devcrumbs-get <ID>` → prüfe linked_items
...
**Lösung**: IMMER `devcrumbs-update` oder `devcrumbs-link` verwenden
...
**Problem**: Es gibt kein `devcrumbs-unlink` Tool!
...
2. **Nach Link-Erstellung**: Validiere mit `devcrumbs-trace`
...
- `.devcrumbs/HIERARCHY.md` - Vollständige Hierarchie-Definition
- `.devcrumbs/config.json` - Projekt-Konfiguration mit hierarchies
- `scripts/fix-spike-links.sh` - Beispiel für Link-Korrektur
- `packages/shared/src/core/` - Verfügbare Core-Funktionen

---

**REMEMBER**: DevCrumbs ist ein **Traceability-System**. Jeder Link hat Bedeutung. Falsche Links = Falsche Traceability = Chaos!

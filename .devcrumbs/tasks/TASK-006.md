...
1. **devcrumbs.copyId** - Copy work item ID to clipboard:
...
2. **devcrumbs.revealInExplorer** - Show item in file explorer:
   - Resolves item type folder dynamically
   - Calls VS Code's native `revealInExplorer` command
   - Navigates to `.devcrumbs/{type}s/{id}.md`
   - Error handling for missing items

3. **devcrumbs.editProperties** - Quick property editor:
...
1. **view/item/context**: 5 contextual actions
   - Condition: `view == devcrumbs.itemsView && viewItem == workItem`

Implemented comprehensive interactive command handlers for VS Code extension with full @devcrumbs/shared integration.

**Commands Implemented:**

1. **devcrumbs.addItem** - Multi-step work item creation wizard:
...
2. **devcrumbs.openItem** - Open work item markdown files:
...
3. **devcrumbs.updateStatus** - Interactive status management:
...
4. **devcrumbs.searchItems** - Full-text search functionality:
...
5. **devcrumbs.showStatus** - Project statistics dashboard:
   - Total item count
...
**Integration:**
- Uses @devcrumbs/shared functions: `addItem`, `getItem`, `updateItem`, `listItems`
...
- Registered `devcrumbs.searchItems` with search icon
- Registered `devcrumbs.showStatus` with graph icon
- Added icons to `devcrumbs.updateStatus` (edit)

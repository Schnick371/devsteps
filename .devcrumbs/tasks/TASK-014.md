...
## Objectives
Add status bar item showing project summary and providing quick access to DevCrumbs features.

## Status Bar Item Features

### Basic Status Display
```typescript
export class DevCrumbsStatusBar {
  ...
  constructor() {
    ...
    this.statusBarItem.command = 'devcrumbs.openDashboard';
    this.update();
  ...
  private buildTooltip(stats: any): string {
    return `DevCrumbs Project Status
      ...
      "statusBar/item": [
        {
          ...
          "when": "statusBarItem == devcrumbs.status"
        },
        {
...
constructor(context: vscode.ExtensionContext) {
  ...
  // Watch .devcrumbs directory for changes
...
```json
{
  "devcrumbs.statusBar.enabled": {
    "type": "boolean",
    "default": true,
    "description": "Show DevCrumbs status in status bar"
  },
  "devcrumbs.statusBar.displayMode": {
    "type": "string",

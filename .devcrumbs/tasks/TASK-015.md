...
export function activate(context: vscode.ExtensionContext) {
  const logger = new DevCrumbsLogger();
  logger.info('DevCrumbs extension activated');
...
```json
{
  "commands": [
    {
      "command": "devcrumbs.showOutput",
      "title": "DevCrumbs: Show Output Channel"
    },
    {
      "command": "devcrumbs.clearOutput",
      "title": "DevCrumbs: Clear Output Channel"
    }
  ]
}
```
...
### Show on Error
```typescript
// In settings: devcrumbs.logging.showOutputOnError
...
## Acceptance Criteria
- ✅ Output channel created with 'DevCrumbs' name

...
| STANDARD    | `analyst-context` + `analyst-internal` + `analyst-risk`                                                           | `aspect-constraints` + `aspect-impact` + `aspect-staleness`                    | → `exec-planner` → `exec-impl` → `exec-test` → `gate-reviewer`              |

...
3. **Same-phase dispatches fire simultaneously** — ALL `runSubagent` calls for the same ring in ONE tool-call batch (single response turn). Waiting between calls = SEQUENTIAL = FORBIDDEN (I-8).

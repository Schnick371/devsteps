...
> **VS Code 1.119.0**: Nested `runSubagent` dispatch is supported (depth-5); however, `allowInvocationsFromSubagents` defaults to `false`. Conductors (`exec-impl`, `exec-test`, `exec-doc`) require this enabled to dispatch their worker pools. Agents with `dispatch_role: leaf` NEVER call `runSubagent`. Empirical validation of the conductor model is recommended before Ring-4 worker dispatch is used in production (see SPIKE-281-R4).

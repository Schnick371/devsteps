...
> **VS Code 1.119.0**: Nested `runSubagent` dispatch is supported. Conductors (`exec-impl`, `exec-test`, `exec-doc`) dispatch workers via the `agent` tool (in `tools:`) + `agents:` frontmatter list — no extra setting required. Note: `chat.subagents.allowInvocationsFromSubagents` is only needed for self-referential agents (those listing themselves in `agents:`). Agents with `dispatch_role: leaf` NEVER call `runSubagent`. Empirical validation via OTel tracing is recommended before assuming conductor dispatch works in all production scenarios (see SPIKE-064).
...
| Pre-release                   | `devsteps-release-next`       | coord               |
| Production release            | `devsteps-release`            | coord               |
